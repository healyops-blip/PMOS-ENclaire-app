...
| `/var/lib/pomi/storage` | private originals and generated reports | `pomi:pomi`, `0700` |
...
sudo install -d -o pomi -g pomi -m 0700 /var/lib/pomi /var/lib/pomi/storage /var/backups/pomi
...
sudo nginx -t
sudo systemd-analyze verify /etc/systemd/system/pomi-api.service /etc/systemd/system/pomi-worker.service /etc/systemd/system/pomi-backup.service /etc/systemd/system/pomi-backup.timer
...
6. Run `sudo systemctl restart pomi-api.service pomi-worker.service`.
...
1. Stop `pomi-api.service` and `pomi-worker.service`.
...
```bash
systemctl is-enabled pomi-api.service pomi-worker.service pomi-backup.timer
systemctl is-active pomi-api.service pomi-worker.service pomi-backup.timer
curl --fail --silent https://api.healy1012-ops.top/health/ready
sudo -u pomi /opt/pomi/current/backend/.venv/bin/python /opt/pomi/current/deploy/scripts/auth_smoke.py
```
...
sudo systemctl status pomi-worker.service
