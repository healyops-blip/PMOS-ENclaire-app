...
- Flutter 在自述确认或明确跳过后先调用 `POST /api/reports/preflight`；存在缺失区块时必须展示清单并由用户显式确认，随后才以 `confirm_incomplete=true` 调用 `POST /api/reports`。
- 后端只读取当前 UID 的正式确认表；OCR 草稿、失败任务和仅有 `ocr_result` 而没有正式业务记录的数据不会进入报告。
...
- `GET /api/reports` 按最新成功版本排序，并用当前确认数据重新计算摘要以返回 `has_updates`；`GET /api/reports/{report_id}` 返回当前 UID 的不可变 `snapshot`、`date_sources` 和 `data_freshness`，旧快照内容始终保持生成时状态。
...
创建报告请求：`patient_note_id`（可空）、`include_sections[]`（可空，默认全部）。响应除 `report_id`、`status`、`generated_at`、`snapshot_hash` 外，还包含不可变 `snapshot`、`date_sources` 和 `data_freshness`；历史列表只返回元数据，详情通过 `GET /api/reports/{report_id}` 获取。
