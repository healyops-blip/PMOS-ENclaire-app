...
创建报告请求：`patient_note_id`（可空）、`include_sections[]`（可空，默认全部）。创建响应和历史列表返回 `report_id`、`status`、`generated_at`、`snapshot_hash` 等只读元数据；不可变三层内容通过 `GET /api/reports/{report_id}` 获取。
