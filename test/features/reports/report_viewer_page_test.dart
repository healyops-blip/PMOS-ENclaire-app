...
void main() {
  ...
  testWidgets('local certification read failure never hides report data', (
    tester,
  ) async {
    _smallPhone(tester);
    final reports = DemoReportRepository();
    ...
    await tester.pumpWidget(
      _app(
        ...
        reports,
        _ImageDocumentRepository(),
        _FailingCertificationRepository(),
      ),
    );
    await tester.pumpAndSettle();
    final glucose = find.byKey(const Key('report-summary-metric-glucose'));
...
class _FailingCertificationRepository implements CertificationRepository {
  @override
