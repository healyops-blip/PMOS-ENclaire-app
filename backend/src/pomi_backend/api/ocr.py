...
from pomi_backend.services.ocr_provider import OCRProviderError, OCRProviderRequest, Qwen3VLOCRProvider
...
def recognize(
    ...
    material_type: str = Form(default="outpatient_record"),
    prompt_version: str = Form(default="pomi-ocr-v1"),
    idempotency_key: str = Header(..., alias="Idempotency-Key"),
    consent_version: str | None = Header(default=None, alias="X-External-Processing-Consent-Version"),
) -> dict:
    ...
    if not consent_version:
        raise BusinessError("EXTERNAL_PROCESSING_CONSENT_REQUIRED", "External OCR processing consent is required.", 409)
    settings = request.app.state.settings
    document = DocumentService(session, account, settings.storage_root).upload(
        file, document_type=material_type, idempotency_key=idempotency_key,
        processing_notice_version=consent_version,
    )
    revision = DocumentRepository(session, DocumentService(session, account, settings.storage_root).patient.patient_id).revision(document.id, document.current_revision_id)
    if revision is None:
        raise BusinessError("OCR_FILE_NOT_FOUND", "The uploaded file is unavailable.", 404)
    provider = Qwen3VLOCRProvider(api_base_url=settings.ocr_api_base_url, api_key=settings.ocr_api_key, model=settings.ocr_model or DEFAULT_OCR_MODEL, timeout_seconds=settings.ocr_request_timeout_seconds)
    try:
        response = provider.recognize(OCRProviderRequest(task_id=idempotency_key, material_type=material_type, mime_type=revision.mime_type, file_path=private_path(settings.storage_root, revision.storage_path), file_name=document.original_file_name, uploaded_at=document.uploaded_at.isoformat(), file_hash=revision.file_hash))
    except OCRProviderError as error:
        raise BusinessError(error.code, error.safe_message, 504 if error.category == "timeout" else 503, details={"retryable": error.retryable}) from error
...
def _normalize_algorithm_payload(payload: dict, original_file_name: str) -> dict:
    ...
    if isinstance(payload.get("draft"), dict):
        ...
        payload = {
            ...
            "examinations": draft.get("examinations") or [
                {
                    "item_name": item.get("item_name"),
                    "value": item.get("raw_value") if item.get("raw_value") is not None else item.get("numeric_value"),
            ...
            "medication_suggestions": draft.get("medication_suggestions") or draft.get("orders", []),
